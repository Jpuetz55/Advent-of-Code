...
fn hand_array_vec_from_combo_vec() -> Vec<[usize; 13]> {
...
#[test]
fn get_max_rank_from_hand_array_vec() {
    let mut max_rank = 0;
    let hand_array_vec = hand_array_vec_from_combo_vec();
    for hand_array in hand_array_vec {
        let rank = hand_to_hand_rank(hand_array);
        if rank > max_rank {
            max_rank = rank;
        }
    }
