...
// Game 15: 2 red, 4 green, 4 blue; 5 red; 5 green, 2 red, 2 blue; 5 green, 1 blue, 7 red";
    ...
    for game in games {
        ...
        for letter in game.chars() {
            if i >= 7
            {
                ...
                {                                      
                    ...
                    if !game.chars().nth((i + 1).try_into().unwrap()).unwrap().is_digit(10) //next character sin't a digit. we have all digits now.
                    { 
        ...
        print!("Game Total Prev + {} = {}----", red_max_count * green_max_count * blue_max_count, game_total);
        print!("Blue Max {}----", blue_max_count);
        print!("Green Max {}----", green_max_count);
